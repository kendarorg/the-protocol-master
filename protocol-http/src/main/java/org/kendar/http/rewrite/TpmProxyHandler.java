 ...
 * Proxy handler.
 */
public class TpmProxyHandler extends SeleniumProxyHandler {
    protected static final Logger logger = LoggerFactory.getLogger(TpmProxyHandler.class);
    ...
    protected long proxyPlainTextRequest(final URL url, final String pathInContext, final String pathParams, final HttpRequest request,
        ...
        try {
            ...
            if (forbiddenUrls.stream().anyMatch(founded->urlStr.startsWith(founded))){
            ...
            httpReq.setRequestCallback(new RequestCallback() {
                ...
                public void reportError(final Exception e) {
                    TpmProxyHandler.reportError(e, url, response);
                }
            });

            if(this.responderInterceptor!=null){
                var res = this.responderInterceptor.intercept(httpReq);
                if(res!=null){
                    request.setHandled(true);
                    return res.getEntry().getResponse().getBodySize();
                }
            }
    ...
    public void setResponderInterceptor(CustomResponderInterceptor responderInterceptor) {
        this.responderInterceptor = responderInterceptor;
    }
}